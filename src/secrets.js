...
export function initSecrets(map) {
    ...
    function renderList(visible) {
        ...
        const header = document.createElement('button'); header.type = 'button'; header.className = 'sample-section-header secret-section-header'; header.setAttribute('aria-expanded', String(expanded)); header.innerHTML = `<span>Secrets (${visible.length}/${secrets.length} shown)</span><span class="sample-section-toggle">${expanded ? '▼' : '▶'}</span>`;
